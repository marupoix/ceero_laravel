...
export function OrderDetailsModal({ isOpen, closeModal, data }: Props) {
                                    ...
                                    <tbody>
                                        {data.products.map((product: any) => (
                                            <tr className="h-10" key={product.id}>
                                                ...
                                                <td className="text-right px-2">{`₱${product.pivot.price}`}</td>
                                                <td className="text-right px-2">{product.pivot.quantity}</td>
                                                <td className="text-right px-2">{`₱${(product.pivot.price * product.pivot.quantity).toFixed(2)}`}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>    
                                <div className="w-full">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <input
                                            ...
                                            value={`₱${Number(data.cash).toFixed(2)}`}
                                            readOnly
                                        />
                                    </div>
                                    <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <input
                                            className="p-2 rounded text-right focus:outline-none md:w-1/2 w-full"
                                            value={`₱${Number(data.change).toFixed(2)}`}
                                            readOnly
                                        />
                                    </div>
                                    <div className="border-y border-dotted border-gray-600 py-2">
                                        <div className="flex justify-between items-center">
                                            <label>Sold To: </label>
                                            <input
                                                className="p-2 rounded text-right focus:outline-none md:w-1/2 w-full"
